...
async function carregarJogos() {
    try {
        ...
        // Não mostrar alerta para evitar mensagens de erro desnecessárias 
        // em páginas que não precisam carregar jogos diretamente
    }
}
...
function gerarEstrelas(avaliacao) {
    let estrelas = '';
    for (let i = 1; i <= 5; i++) {
        estrelas += `<span class="pixel-star ${i <= avaliacao ? 'filled' : ''}"></span>`;
    }
    return estrelas;
}
...
function salvarJogo(evento) {
    ...
    if (jogoId) {
        atualizarJogo(evento, jogoId);
    } else {
        adicionarJogo(evento);
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const formulario = document.getElementById('form-jogo');
    if (formulario) {
        formulario.addEventListener('submit', salvarJogo);
    }
    ...
    const addBtn = document.querySelector('.add-game-btn');
    if (addBtn) {
        addBtn.style.display = currentUser ? 'inline-block' : 'none';
    }
});